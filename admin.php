...
elseif (empty($_POST['username']) || empty($_POST['password']) || empty($_POST['type']))
{
	...
	echo"<script type='text/javascript'>window.location ='admin/?adm=home'</script>";
}
else
{
	...
	//echo"$username, $password, $type";die;
		$query		= "SELECT username * FROM administrator WHERE username='$username' AND password='$password'";
	...
	if($rowCheck > 0)
	{
		...
		{
			$_SESSION['administrator'] = $dataAdm[0]>$dataAdm[1];
			echo "<script type='text/javascript'>window.location ='admin/?adm=home'</script>";
		}
	}	
	else 
